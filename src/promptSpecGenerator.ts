  ...
  constructor(private readonly files: string[], private readonly promptTemplateFileName: string) {
    ...
    for (let i=0; i < this.files.length; i++){
      const file = this.files[i];
       this.promptSpecs.push(...processFile(file))
    }
    for (const promptSpec of this.promptSpecs){
      ...
      const codeWithPlaceholder = 
         getText(code, 1, 0, promptSpec.location.startLine, promptSpec.location.startColumn) + 
         "<PLACEHOLDER>" + 
    ...
    // create directory "prompts" if it does not exist
    const dir = path.join('.', 'prompts');
...
function createPromptSpecForIf(file: string, path: any) : PromptSpec {
...
function createPromptSpecForSwitch(file: string, path: any) {
...
function createPromptSpecForWhile(file: string, path: any) {
  const test = path.node.test;
  const loc = new SourceLocation(file, test.loc!.start.line, test.loc!.start.column, test.loc!.end.line, test.loc!.end.column);
  return new PromptSpec(file, "while", "test", loc, loc.getText());
}

function createPromptSpecForDoWhile(file: string, path: any) {
  const test = path.node.test;
  const loc = new SourceLocation(file, test.loc!.start.line, test.loc!.start.column, test.loc!.end.line, test.loc!.end.column);
  return new PromptSpec(file, "do-while", "test", loc, loc.getText());
}

function createPromptSpecsForFor(file: string, path: any) {
...
function createPromptSpecsForForIn(file: string, path: any){
...
function createPromptSpecsForForOf(file: string, path: any){
  const left = path.node.left;
  const right = path.node.right;
  const leftLoc = new SourceLocation(file, left!.loc!.start.line, left!.loc!.start.column, left!.loc!.end.line, left!.loc!.end.column);
  const rightLoc = new SourceLocation(file, right!.loc!.start.line, right!.loc!.start.column, right!.loc!.end.line, right!.loc!.end.column);
...
function createPromptSpecsForCall(file: string, path: any) : Array<PromptSpec> {
...
function processFile(file: string) : Array<PromptSpec> {
  ...
  traverse(ast, {
    enter(path) {
      ...
      if (path.isIfStatement()) {
        promptSpecs.push(createPromptSpecForIf(file, path));
      } if (path.isSwitchStatement()) {
        promptSpecs.push(createPromptSpecForSwitch(file, path));
      } else if (path.isWhileStatement()) {
        promptSpecs.push(createPromptSpecForWhile(file, path));
      } else if (path.isDoWhileStatement()) {
        promptSpecs.push(createPromptSpecForDoWhile(file, path));
      } else if (path.isForStatement()) {
        promptSpecs.push(...createPromptSpecsForFor(file, path));  
      } else if (path.isForInStatement()) {
        promptSpecs.push(...createPromptSpecsForForIn(file, path)); 
      } else if (path.isForOfStatement()) {
        promptSpecs.push(...createPromptSpecsForForOf(file, path)); 
      } else if (path.isCallExpression() && path.node.loc!.start.line === path.node.loc!.end.line) { // for now, restrict to calls on a single line
        promptSpecs.push(...createPromptSpecsForCall(file, path)); 
      }
    }
  }); 
  return promptSpecs;
