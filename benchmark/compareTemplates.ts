...
const templates = ['full', 'onemutation', 'noexplanation', 'noinstructions', 'gen.system prompt', 'basic'];
...
function computeTotals(baseDir: string, run: string) {
  ...
  for (const projectName of projectNames) {
    const {total, nrKilled, nrSurvived, nrTimedout} = findProjectData(baseDir, run, projectName);
    totalMutants += total;
...
function generateTable(baseDir: string, runs: string[]) : void {
  ...
  for (const run of runs) {
    const {totalMutants, totalKilled, totalSurvived, totalTimedout} = computeTotals(baseDir, run);
    row += ' & ' + totalMutants + ' & ' + totalKilled + ' & ' + totalSurvived + ' & ' + totalTimedout;
  }
  row += ' \\\\\n';
  latexTable += row;
