...
export const testProjectPath =
  "./test/input/testProject/countries-and-timezones";

export function setContainsPrompt(
  prompts: Set<Prompt>,
  prompt: Prompt
): boolean {
...
export function findExpectedPrompts(promptDir: string): Set<Prompt> {
  ...
  for (const expectedPromptFile of expectedPromptFiles) {
    ...
    if (
      expectedPromptFile.indexOf("completion") === -1 &&
      expectedPromptFile.endsWith(".json")
    ) {
      const jsonObj = JSON.parse(
        fs.readFileSync("./test/input/prompts/" + expectedPromptFile, "utf8")
      );
...
export function findExpectedCompletions(promptId: number): Set<Completion> {
  ...
  for (const expectedCompletionFile of expectedCompletionFiles) {
    if (
      expectedCompletionFile.indexOf(`prompt_${promptId}_completion`) !== -1 &&
      !expectedCompletionFile.endsWith(".txt")
    ) {
      const jsonObj = JSON.parse(
        fs.readFileSync(
          "./test/input/prompts/" + expectedCompletionFile,
          "utf8"
        )
      );
